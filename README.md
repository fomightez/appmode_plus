...
**NOTE:** Presently gradually transitioning this from similar to [my fork of the binder-examples/appmode repo](https://github.com/fomightez/appmode) to my version with Seaborn, VPython including to try to see why [borked_appmode_plus](https://github.com/fomightez/borked_appmode_plus) fails to work in 'appmode' even though I thought I instituted a fix that works in both [my fork of the binder-example/appmode repo](https://github.com/fomightez/appmode) and [a simpler appmode example I had from demonstrating a very old fix](https://github.com/fomightez/simple_appmode_binder) by adding a setting in `jupyter_notebook_config.py` to allow hidden_files to be saved. I think something I added messed it up but I cannot tell by looking what it is and so this way will gradually change it until it breaks and indicates the problematic change.
